...
///            改变该字段的设置以后，本地摄像头的预览画面方向也会发生改变，请调用 TXLivePush 的
///            setRenderRotation 进行矫正。
...
///【特别说明】调用 TXLivePusher 的 pausePush()
///接口，会暂停摄像头采集并进入垫片推流状态，如果该状态一直保持，
...
///【特别说明】此值设置需与调用 sendVideoSampleBuffer 时传入的 SampleBuffer
///的宽高比一致，否则会引起画面变形。
///            如果指定 autoSampleBufferSize 为 YES，则不需要设置该字段。
@property(assign) CGSize sampleBufferSize;

///【字段含义】仅开启自定义采集时有效，YES 代表编码分辨率等于输入的 SampleBuffer
///的分辨率，默认值：NO。
...
///【字段含义】是否开启噪声抑制（注意：早期版本引入了拼写错误，考虑到接口兼容一直没有修正，正确拼写应该是
/// ANS） 【推荐取值】NO：ANS 对于直播环境中由其它设备外放的音乐是不友好的，通过 playBGM
///设置的背景音不受影响。 【特别说明】如果直播场景只有主播在说话，ANS
///有助于让主播的声音更清楚，但如果主播在吹拉弹唱，ANS 会损伤乐器的声音。
@property(nonatomic, assign) BOOL enableNAS;

///【字段含义】是否开启回声抑制
///【推荐取值】NO：回声抑制会启用通话模式音量，导致音质变差，非连麦场景下请不要开启。
///【特别说明】只有在连麦模式下才需要开启
/// AEC，如果是普通的直播，将主播的手机和观众的手机放在一起所产生的啸叫是正常现象。
...
///【字段含义】是否前置摄像头，待废弃，建议直接使用 TXLivePusher 的 frontCamera 属性和 switchCamera
///函数。
