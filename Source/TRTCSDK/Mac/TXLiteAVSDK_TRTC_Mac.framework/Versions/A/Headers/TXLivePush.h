 ...
 *
 * 当您通过 sendAudioSampleBuffer 向 SDK 调用各种类型的声音数据时，SDK
 * 内部会进行混流，否则只会发送一路的声音数据。
 *
 * @param sampleBuffer 采集到的声音 sampleBuffer
 * @param sampleBufferType RPSampleBufferTypeAudioApp：ReplayKit 采集到的 App
 * 声音；RPSampleBufferTypeAudioMic：ReplayKit 采集到的麦克风声音。
 */
#if TARGET_OS_IPHONE
 ...
 * 7.4 要求 SDK 发送静音数据
 *
 * 该函数配合 sendAudioSampleBuffer 使用，在 InApp
 * 类型录制切后台场合时需要调用，系统屏幕录制不需要。
 *
