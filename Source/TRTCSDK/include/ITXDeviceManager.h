 ...
 */
     ...
     *         - true: following. When the default audio device of the system changes, the SDK immediately switches the audio device.
     *         - false：not following. The SDK switches to the system default audio device only after the audio device used by the SDK is removed or a new audio device is inserted as the system default device.
     */
...
//
/////////////////////////////////////////////////////////////////////////////////
/// @name  Disused APIs (the corresponding new APIs are recommended)
/// @{

/**
 * Setting the system volume type (for mobile OS)
 *
 * @deprecated This API is not recommended after v9.5. Please use the `startLocalAudio(quality)` API in {@link TRTCCloud} instead, which param `quality` is used to decide audio quality.
 */
#if __ANDROID__ || (__APPLE__ && TARGET_OS_IOS)
