...
struct TRTCPublishTarget {
    ...
    ///**Note:** If you need to use more than one publishing mode, you can call {@link startPublishMediaStream} multiple times and set `TRTCPublishTarget` to a different value each time.
    ///**Note:** You can use one mode each time you call the {@link startPublishMediaStream}) API. To modify the configuration, call {@link updatePublishCDNStream}.
    ...
    /// streams that are transcoded).
    ///**Note:** If you set the subscription mode ({@link setDefaultStreamRecvMode}) to auto before room entry, users whose streams are not transcoded will receive the transcoded stream automatically and will unsubscribe from the users whose streams
    /// are transcoded. You call {@link muteRemoteVideoStream} and {@link muteRemoteAudio} to unsubscribe from the transcoded stream.
    ///**Note:** The {@link muteAllRemoteAudio} and {@link muteAllRemoteVideoStreams} APIs do not apply to the data published by the robot.
...
struct TRTCVideoLayout {
    ...
    ///**Note:** You need to specify the `userId` parameter in `fixedVideoUser`.
    ///**Notes:**
...
struct TRTCStreamEncoderParam {
    ...
    ///**Note:** The audio sample rates supported by HE-AAC and HE-AACv2 are 48000, 44100, 32000, 24000, and 16000.
    ///**Note:** When HE-AACv2 is used, the output stream can only be dual-channel.
